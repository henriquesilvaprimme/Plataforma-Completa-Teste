...
  // --- LÓGICA DE PERMISSÃO (Quem vê o quê) ---
...
    // 3. Filtra leads que NÃO são renovações (seguros novos)
...
      stats.totalLeads++;
      const s = lead.status ?? '';
      // CORREÇÃO: Acesso robusto à seguradora (Seguradora ou insurer)
      const segNormalized = (lead.Seguradora || lead.insurer || '').toString().trim().toLowerCase();
...
        // CORREÇÃO: Acesso robusto ao prêmio e comissão
        const premio = safeParseFloat(lead.PremioLiquido || lead.premioLiquido);
        const comissao = safeParseFloat(lead.Comissao || lead.comissao);

        stats.totalPremioLiquidoLeads += premio;
        stats.somaTotalPercentualComissaoLeads += comissao;
...
      // CORREÇÃO: Acesso robusto à seguradora (Seguradora ou insurer)
      const segNormalized = (renovacao.Seguradora || renovacao.insurer || '').toString().trim().toLowerCase();
...
        // CORREÇÃO: Acesso robusto ao prêmio e comissão
        const premio = safeParseFloat(renovacao.PremioLiquido || renovacao.premioLiquido);
        const comissao = safeParseFloat(renovacao.Comissao || renovacao.comissao);

        stats.premioLiquidoRenovados += premio;
        stats.somaComissaoRenovados += comissao;
...
    // Converte a string de porcentagem para float (para cálculo)
    const floatPercentage = parseFloat(String(percentage).replace(',', '.')) || 0;
    const strokeDashoffset = circumference - (floatPercentage / 100) * circumference;
